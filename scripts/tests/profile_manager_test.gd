...
func _run() -> void:
	...
	_expect(not manager.is_unlocked(&"tutorial_mission_2"), "Default profile unlocked dependent mission")
	...
	_expect(restored.is_completed(&"tutorial_mission_1") and restored.is_unlocked(&"tutorial_mission_2"), "Profile state changed after restart")
